...
│   └── _Libraries/              # Resolved library references (generated by TcBuild/XAE, gitignored)
...
├── specs/                       # Specification documents (design / plans / steering)
│   ├── design/index.md          # Design docs (repository structure, tracking, axis unification, ScopeView .svdx format, ...)
│   ├── plans/index.md           # Dated investigation/work logs
│   └── steering/index.md        # Cross-cutting operational knowledge
└── README.md
```
...
- **`FB_BaseTelescopeControl`** — abstract base class for all telescope types.
  ...
  `Track`, `Slew`, `Park`, `Stop`, `PowerOn`), coordinate handling and MQTT
...
- **`FB_PointingModelForward`** — Tpoint-style parametric pointing model
  (9 terms: AOFF, BNP, AN_A, AE_A, NPAE, EOFF, AN_E, AE_E, TF), producing
  Az/El pointing offsets (not velocities — those come from the tracking
  functions below).
...
- **`FB_ButtonEnable`** — toggles a latched output on each button press
  (`SET` flips it, `RESET` clears it); not a debouncer.
- **`FB_Horn` / `E_Horn`** — warning-horn control.
- **`FB_LightTimer`**, **`FB_TONTP`** — timer blocks.
- **`F_YREAL`** — linear scaler (`y = m*x + b`) from an input range to an
  output range, with optional clamping; **`NCError_TO_STRING`** — maps
  TwinCAT NC error codes to readable text; **`GVL_Math`** — mathematical
  constants.
...
- `ST_TelescopeConfig` — telescope configuration structure shared with IAG50cm
  (identical copy in both projects; not yet unified into a single definition).
...
- Versioned with git tags (`vMAJOR.MINOR.PATCH`); the library version is
  bumped per release (e.g. *0.4.1* added scientific-notation support in
  `F_IsNumericValue`).
- `specs/` holds lightweight specification documents: design docs (e.g. the
